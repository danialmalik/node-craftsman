...
function handleRequest(request, response) {
    ...
    var pageContent = '<html>' +
        ...
        '<input type="submit" value="Add content" />' + '</form>' +
        '<div>' +
        ...
        '<input type="submit" value="Filter content" />' + '</form>' +
        '</body>' +
        '</html>';
...
function getContentsFromDatabase(filter, callback) {
    var connection = mysql.createConnection({
        host: 'localhost', user: 'root', password: 'abcd1234', database: 'node'
    });
    var query;
    var resultsAsString = '';
    if (filter) {
    ...
    query.on('result', function (result) {
        resultsAsString += 'id: ' + result.id; resultsAsString += ', content: ' + result.content;
        resultsAsString += '\n';
    });
